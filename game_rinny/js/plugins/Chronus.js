 ...
 * @author トリアコンタン/<译>公孖。狼
 *
 * @param 月ごとの日数配列
 * @desc 自由设定各月份日数、按英文逗号划分。
 * @default 31,28,31,30,31,30,31,31,30,31,30,31
 *
 * @param 月名配列
 * @desc 自由设定各月份名称、按英文逗号划分。
 * @default Jan.,Feb.,Mar.,Apr.,May.,Jun.,Jul.,Aug.,Sep.,Oct.,Nov.,Dec.
 *
 * @param 曜日配列
 * @desc 自由设定各单日星期名称、按英文逗号划分。
 * @default (日),(月),(火),(水),(木),(金),(土)
 *
 * @param 自然時間加算
 * @type number
 * @desc 设定现实时间每1秒加算游戏时间分针值。事件执行内无效。
 * @default 5
 *
 * @param 自然時間加算間隔
 * @type number
 * @desc 设定游戏时间加算标准(帧数)。如1帧=1/60秒
 * @default 60
 *
 * @param 場所移動時間加算
 * @type number
 * @desc 设定单次场所移动加算游戏时间分针值。
 * @default 30
 *
 * @param 戦闘時間加算(固定)
 * @type number
 * @desc 设定单回战斗加算游戏时间分针值。
 * @default 30
 *
 * @param 戦闘時間加算(ターン)
 * @type number
 * @desc 设定单个战斗回合加算游戏时间分针值。
 * @default 5
 *
 * @param 年のゲーム変数
 * @type variable
 * @desc 指代编号的游戏变量自动对应年份。
 * @default 0
 *
 * @param 月のゲーム変数
 * @type variable
 * @desc 指代编号的游戏变量自动对应月份。
 * @default 0
 *
 * @param 日のゲーム変数
 * @type variable
 * @desc 指代编号的游戏变量自动对应日期。
 * @default 0
 *
 * @param 曜日IDのゲーム変数
 * @type variable
 * @desc 指代编号的游戏变量自动对应星期。
 * @default 0
 *
 * @param 曜日名のゲーム変数
 * @type variable
 * @desc 指代编号的游戏变量自动对应单日星期。
 * 需要留意的是游戏变量中含文本。
 * @default 0
 *
 * @param 時のゲーム変数
 * @type variable
 * @desc 指代编号的游戏变量自动对应时针值。
 * @default 0
 *
 * @param 分のゲーム変数
 * @type variable
 * @desc 指代编号的游戏变量自动对应分针值。
 * @default 0
 *
 * @param 累計時間のゲーム変数
 * @type variable
 * @desc 指代编号的游戏变量自动对应累计分针值。
 * @default 0
 *
 * @param 累計日数のゲーム変数
 * @type variable
 * @desc 指代编号的游戏变量自动对应累计日期数。
 * @default 0
 *
 * @param 時間帯IDのゲーム変数
 * @type variable
 * @desc 指代编号的游戏变量自动对应时间段。
 * 0:午夜 1:凌晨 2:清晨 3:白昼 4:黄昏 5:暗夜
 * @default 0
 *
 * @param 天候IDのゲーム変数
 * @type variable
 * @desc 指代编号的游戏变量自动对应天气特征。
 * 0:なし 1:落雨 2:刮风 3:飘雪
 * @default 0
 *
 * @param フォーマット時間の変数
 * @type variable
 * @desc 设定「时间规格计算公式」自动对应时间推进。
 * @default 0
 *
 * @param フォーマット時間の計算式
 * @desc 设定「日期规格计算公式」自动对应时日推进。
 * YYYY:年 MON:月名 MM:月 DD:日 等(详情参照帮助说明)
 * @default HH24 * 60 + MI
 *
 * @param 日時フォーマット1
 * @desc 地图中日历窗口首行文本文字。
 ...
 * @param カレンダーフォントサイズ
 * @type number
 * @desc 设定日历字体尺寸。0表示服从默认。
 * @default 0
 *
 * @param カレンダー不透明度
 * @type number
 * @desc 设定日历背景透明度(0-255)。
 * @default 192
 *
 * @param カレンダー枠の非表示
 * @type boolean
 ...
 * @arg time
 * @text 时间
 * @desc 若加算时间需使用特殊字符\v[n]写入文本标签即可。其他字符相同
 * @default 0
 * @type number
 *
 * @command SET_TIME
 ...
 * @command STOP_TIMER
 * @text 计时器・闹铃停止
 * @desc 停止计时以及闹铃功能。停止期间条件满足也无法开关重启。
 *
 * @arg name
 * @text 计时器名称
 * @desc 计时器标识。
 ...
 *
 * 本插件需要系统插件『PluginCommonBase.js』。
 * 『PluginCommonBase.js』置于MZ安装文件夹内
 * 以下为文件路径。
...
(function() {
    ...
    var settings = {
        ...
        timeZone: [
            ...
            {name: '凌晨', start: 0, end: 4, timeId: 0},
            {name: '清晨', start: 5, end: 6, timeId: 1},
            {name: '白昼', start: 7, end: 11, timeId: 2},
            {name: '午后', start: 12, end: 16, timeId: 3},
            {name: '黄昏', start: 17, end: 18, timeId: 4},
            {name: '傍晚', start: 19, end: 21, timeId: 5},
            {name: '午夜', start: 22, end: 24, timeId: 0}
        ],
        /* timeTone:時間帯ごとの色調 */
        timeTone: [
